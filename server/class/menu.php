...
class Menu extends dbobject
{
    ...
    public function menuList($data)
    {
		...
		$columner = array(
			...
			array( 'db' => 'menu_name',  'dt' => 1, 'formatter' => function($d,$row){
                $sql    = "SELECT id from log_table WHERE table_id = '$row[menu_id]' ";
                $result = $this->db_query($sql);
                $count = count($result);
                $count_display = ($count > 0)?"":"display:none";

                return "<b>".$d."</b><div> <span style='cursor:pointer;$count_display' class='badge badge-primary' onclick=\"getpage('log_list.php?table_id=$row[menu_id]','page')\"><i class='fa fa-user'></i> This record was updated $count time(s)</span></div>";
                }),
